...
class FrameStack:
    ...
    def convert_to_stacked_image(self):
        ...
        for i_chunk in range(1, n_chunks + 1):
            ...
            if chunk_end > self.frame_shape[0]:
                chunk_end = self.frame_shape[0] + 1
        ...
        self._save_stack_parameters_as_json()
    ...
    def _save_stack_parameters_as_json(self):
        """
        Saves metadata about the processed image stack to a JSON file.

        Note:
            The output file is overwritten every time this method is called.
        """

        current_path = Path(__file__).resolve().parent

        config_path = current_path.parent / "common"

        stack_parameters = {"stack_path": str(self.inputs.stack_directory / "stack_aligned_array.dat"),
                            "stack_shape": (len(self.frame_list), 4, self.frame_shape[0], self.frame_shape[1]),
                            "stacked_img_path": str(Path(self.output_directory) / "stacked_frame_rgb.tiff"),
        }

        with open(config_path / "parameter_config.json", "w") as json_file:
            json.dump(stack_parameters,json_file, indent=4)
...
def _worker_align_additional_frame_wrapper(arg_tuple):
    ...
    try:
        ...
        # Create a dict to return successful results:
